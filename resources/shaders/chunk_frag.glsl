...
const vec3 colour_fog  = vec3(0.92, 0.95, 1.00);       // 2329 Kid Glove normalised to 1
...
void main() {
  float fogterm = length(coords_camlocal_frag) / 300.0;
  if(fogterm > 1.0) {
    discard;
  }
  ...
  gl_FragColor.rgb = mix(((colour_frag * lambertian) +
