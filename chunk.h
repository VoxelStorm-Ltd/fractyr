...
private:
  ...
  static float constexpr point_density = 0.0006f;                               // points per cubic metre; default 0.0006 = 600 per 100m^3
  ...
  static float constexpr chunk_margin = 0.25f;                                  // how far outside each chunk we compute the voronoi space to avoid discontinuities at edges
  ...
  static unsigned int constexpr num_cells = static_cast<unsigned int>(std::cbrt(expected_points / ideal_points_per_block));
  voro::container con;
